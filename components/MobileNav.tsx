...
const ITEMS = [
  ...
  { label: 'Book', href: '/dashboard/calendar?book=1', icon: 'M12 4v16m8-8H4' },
    ...
    <div className="md:hidden fixed bottom-0 left-0 right-0 bg-warm-100 border-t border-warm-200 px-2 py-2 flex justify-around z-50">
      {ITEMS.map((item) => {
        const active = item.href === '/dashboard/insights'
          ? ['/dashboard/insights', '/dashboard/revenue', '/dashboard/analytics'].some(p => pathname === p || pathname.startsWith(p + '/'))
          : item.href.includes('?')
