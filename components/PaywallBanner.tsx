...
import { getBillingStatus, daysUntil } from '@/lib/billing'
...
export default function PaywallBanner({ subscriptionStatus, subscriptionEndDate }: Props) {
  ...
  if (subscriptionStatus === 'past_due' || subscriptionStatus === 'unpaid' || subscriptionStatus === 'paused') {
    return (
  ...
  if (subscriptionStatus === 'grace_period' || billingStatus === 'grace') {
    return (
  ...
  if (billingStatus === 'blocked') {
    return (
      <div className="flex items-start gap-4 bg-red-950/40 border border-red-700/50 rounded-xl px-4 py-3.5 mb-5">
        <div className="flex-1 min-w-0">
          <div className="text-sm font-semibold text-red-300 mb-0.5">Access suspended</div>
          <div className="text-xs text-red-400/80">Your subscription has ended. Subscribe to restore access.</div>
        </div>
        <button
          onClick={() => router.push('/subscribe')}
          className="flex-shrink-0 bg-od-green hover:opacity-80 text-white font-semibold px-3 py-1.5 rounded-lg text-xs transition-colors whitespace-nowrap"
