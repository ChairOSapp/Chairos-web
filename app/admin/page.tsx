...
export default function AdminPage() {
  ...
  // Verify admin client-side (middleware guards the route, this is belt-and-suspenders)
  useEffect(() => {
      ...
      <div className="max-w-6xl mx-auto px-6 py-6 pb-16">
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-3">
            <div className="text-[10px] font-bold tracking-widest uppercase text-charcoal-400">
              Account Directory
              {!loadingUsers && <span className="ml-2 text-charcoal-500 normal-case font-normal tracking-normal">{filtered.length} of {users.length}</span>}
            </div>
            <button
                ...
                <table className="w-full text-sm" style={{ minWidth: '700px' }}>
                  <thead>
                    <tr className="border-b border-warm-200 bg-warm-200/30">
                      ...
                      <th className="px-4 py-2.5 text-left text-[10px] font-bold tracking-widest uppercase text-charcoal-400"></th>
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.map(u => (
                      <>
                        <tr
                          key={u.id}
                          className="border-b border-warm-200 last:border-0 hover:bg-warm-200/40 transition-colors cursor-pointer"
                          ...
                          <tr key={`${u.id}-detail`} className="border-b border-warm-200 bg-warm-200/20">
                            <td colSpan={6} className="px-4 py-4">
                              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs mb-3">
                                ...
                                <div>
                                  <div className="text-[10px] font-bold tracking-widest uppercase text-charcoal-400 mb-1">Health Check</div>
                                  <HealthBadge status={u.health} />
                                </div>
                              </div>
                ...
                .map(u => (
                  ...
                  >
                    ...
                    <button
                      onClick={() => {
                        setExpandedId(u.id)
                        document.getElementById('account-table-top')?.scrollIntoView({ behavior: 'smooth' })
                      }}
