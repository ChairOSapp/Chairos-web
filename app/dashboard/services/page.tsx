    ...
    <div className="min-h-screen bg-warm-50">
      ...
      <div className="p-6 max-w-3xl mx-auto pb-20 md:pb-0">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="font-serif text-2xl text-charcoal-900 mb-1">Manage Services</h1>
            <p className="text-charcoal-500 text-sm">{shop?.name} · {services.filter(s => s.active).length} active</p>
          </div>
          <button
          ...
          <div className="bg-warm-100 border border-warm-200 rounded-xl p-6 mb-6">
            <div className="flex gap-1 bg-warm-200 rounded-lg p-1 mb-5 w-fit">
              {(['catalog','custom'] as const).map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={`px-4 py-2 rounded-md text-xs font-semibold transition-all ${mode === m ? 'bg-warm-300 text-charcoal-900' : 'text-charcoal-500'}`}>
            ...
            {mode === 'catalog' && (
              <div>
                  ...
                  {CATALOG.map((s, i) => {
                    ...
                    return (
                      <div key={i}
                        ...
                        className={`p-3 rounded-lg border transition-all relative ${added ? 'border-warm-300 bg-warm-200 opacity-40 cursor-not-allowed' : 'border-warm-300 bg-warm-200 hover:border-od-green cursor-pointer'}`}>
                ...
                <button onClick={() => { resetForm(); setShowForm(false) }}
                  className="mt-4 text-sm text-charcoal-500 hover:text-charcoal-900 transition-colors">
                  Close
                </button>
              </div>
            )}
            ...
            {mode === 'custom' && (
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div>
                    <label className="block text-xs font-semibold tracking-widest uppercase text-charcoal-400 mb-2">Duration (mins)</label>
