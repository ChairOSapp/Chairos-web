...
// Square webhook events land here because the Square Developer Console has this
// URL registered as the webhook notification URL. Handle them the same as /api/square/webhook.
const admin = createClient(
  process.env.NEXT_PUBLIC_SUPABASE_URL!,
  process.env.SUPABASE_SERVICE_ROLE_KEY!
)

function verifySquareSig(body: string, signature: string, key: string, url: string): boolean {
  const hash = createHmac('sha256', key).update(url + body).digest('base64')
...
export async function POST(req: NextRequest) {
  ...
  if (process.env.SQUARE_WEBHOOK_SIGNATURE_KEY) {
    if (!verifySquareSig(body, signature, process.env.SQUARE_WEBHOOK_SIGNATURE_KEY, req.url)) {
  ...
  if (event.type === 'payment.updated') {
    ...
    if (payment?.id && payment?.reference_id) {
      const paymentStatus = payment.status === 'COMPLETED' ? 'paid' : payment.status === 'FAILED' ? 'failed' : null
      if (paymentStatus) {
        await admin.from('appointments').update({
          payment_status: paymentStatus,
          square_payment_id: payment.id,
          amount_paid: paymentStatus === 'paid' ? (payment.amount_money?.amount ? Number(payment.amount_money.amount) / 100 : null) : null,
