...
var ownerDailyBrief = schedules_exports.task({
  ...
  run: /* @__PURE__ */ __name(async () => {
    ...
    console.log("[owner-daily-brief] raw shops:", JSON.stringify(shops));
    ...
    console.log("[owner-daily-brief] active shops found:", activeShops.length);
    let briefsCreated = 0;
    for (const shop of activeShops) {
      try {
        const shopId = shop.id;
        const ownerId = shop.owner_id;
        const ownerProfile = Array.isArray(shop.profiles) ? shop.profiles[0] : shop.profiles;
