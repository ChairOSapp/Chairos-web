...
var clientLapseDetection = schedules_exports.task({
  ...
  run: /* @__PURE__ */ __name(async () => {
    ...
    for (const [, client] of clientMap) {
      ...
      const { data: existingAlert } = await supabase.from("lapse_alerts").select("id").eq("shop_id", client.shop_id).eq("client_phone", client.client_phone).is("resolved_at", null).maybeSingle();
